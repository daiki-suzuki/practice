...
bool PopulateCommandList()
{
	...
	handleSRV.ptr += g_cbvSrvDescriptorSize;
	g_commandList->SetGraphicsRootDescriptorTable( 0, handleCBV );
	g_commandList->SetGraphicsRootDescriptorTable( 1, handleSRV );
...
bool CreateRootSignature()
{
	...
	param[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_GEOMETRY;
...
bool CreatePipelineStateObject()
{
	...
	UINT compileFlag = 0;
#endif
	if( FAILED(D3DCompileFromFile(L"shader.hlsl", nullptr, nullptr, "VSMain", "vs_5_0", compileFlag, 0, &vertexShader, &error) ) )
	{
		return false;
	}
	if( FAILED(D3DCompileFromFile(L"shader.hlsl", nullptr, nullptr, "GSMain", "gs_5_0", compileFlag, 0, &geometryShader, &error) ) )
	{
		return false;
	}
	if( FAILED(D3DCompileFromFile(L"shader.hlsl", nullptr, nullptr, "PSMain", "ps_5_0", compileFlag, 0, &pixelShader, &error) ) )
	{
		return false;
	}
	...
	rasterrizerStateDesc.CullMode = D3D12_CULL_MODE_NONE;
...
bool CreateCbvSrv()
{
	//定数バッファ、シェーダーリソースビュー用の記述子ヒープ作成
	{
		...
		desc.NumDescriptors = 2;	//定数バッファとシェーダリソースビューを作成背売るので2つ
	...
	ComPtr<ID3D12Resource> textureUploadHeap;
	{
		...
		subResourceData.RowPitch = 512 * 4;
